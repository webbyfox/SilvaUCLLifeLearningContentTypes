...
# Others
from interfaces import ICourseSubject, ICourseSubjectVersion
...
class CourseSubject(CatalogedVersionedContent):
    """
    Simple Course subject object for Course
    """
    implements(ICourseSubject)
    security = ClassSecurityInfo()
    meta_type = "Course Subject"
    __doc__ = "Course Subject Object"
    ...
    def save_title_and_metadata(self, html):
        ...
        if ICourseSubjectVersion.providedBy(version):
            version.set_shortdescription(handler.metadata['shortdescription'][0])
            version.set_image(handler.metadata['image'][0])
            version.set_casestudy(handler.metadata['casestudy'][0]) 
        version.set_title(handler.title)

class CourseSubjectVersion(CatalogedVersion):           
    """Base class for course object versions
    """        
    implements(ICourseSubjectVersion)
    security = ClassSecurityInfo()
    meta_type = "Course Subject Version"

    def __init__(self,id):
        super(CourseSubjectVersion, self).__init__(id)
